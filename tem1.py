        ...
        if result[i] == 10:
            ...
            if result[i] != global_result[i]:
                ...
                for j in range(8):
                    count = 0
                    while True:
                        tex = font.render(param[i], True, colour[j % 2], None)
                        ...
                        if count > j * 50:
                            break
            tex = font.render(param[i], True, (0, 255, 0), None)
            tex_rect = tex.get_rect()
            tex_rect.center = (3 * width // 4, pos)
            ...
            if result[i] != global_result[i]:
                ...
                #     if count > 1000:
                #         break
            tex = font.render(param[i], True, (255, 0, 0), None)
            tex_rect = tex.get_rect()
            tex_rect.center = (3 * width // 4, pos)
...
while True:
    ...
    for i in range(6):
        ...
        pygame.draw.rect(screen, (153, 255, 153), dash_rect, 3)
    ...
    for i in range(6):
        ...
        check_strings(txtinp_disp, words, screen)
        pos += offset
